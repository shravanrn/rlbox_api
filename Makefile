...
out/x32/test: mkdir_out $(CURDIR)/test.cpp $(CURDIR)/rlbox.h $(CURDIR)/libtest.cpp $(CURDIR)/libtest.h
	g++ -m32 $(NACL_INCLUDES) $(WASM_INCLUDES) -std=c++14 -gstabs+ -Wall $(CURDIR)/test.cpp $(CURDIR)/libtest.cpp -Wl,--export-dynamic -ldl $(NACL_LIBS_32) -o $@
...
out/x64/test: mkdir_out $(CURDIR)/test.cpp $(CURDIR)/rlbox.h $(CURDIR)/libtest.cpp $(CURDIR)/libtest.h
	g++ $(NACL_INCLUDES) $(WASM_INCLUDES) -std=c++14 -gstabs+ -Wall $(CURDIR)/test.cpp $(CURDIR)/libtest.cpp -Wl,--export-dynamic -ldl $(NACL_LIBS_64) $(WASM_LIBS_64) -o $@
